...
var BingoBoard = /** @class */ (function () {
    ...
    BingoBoard.prototype.print = function () {
        var comb = (0, lodash_1.zip)(this.board, this.markings);
        var chunked = (0, lodash_1.chunk)(comb, this.size);
        for (var _i = 0, chunked_1 = chunked; _i < chunked_1.length; _i++) {
            var line = chunked_1[_i];
            var l = line.map(function (li) { return "".concat(li[0], " ").concat(li[1]); });
            console.log(l);
        }
    };
...
var recurse = function (nums, boards) {
    if (nums.length === 0)
        throw "fuck";
    var head = nums[0];
    ...
    for (var index in boards) {
        ...
        if (board.markNumber(head)) {
            if (boards.length - wonIndex.length === 1)
                return [board, head];
            wonIndex.push(index);
        }
    }
    var nextBoards = boards.filter(function (_, i) { return !wonIndex.includes(i.toString()); });
    return recurse(tail, nextBoards);
};
var _a = recurse(inputs, boards), winner = _a[0], lastnum = _a[1];
console.log(lastnum);
